        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[var(--text-muted)] mt-1">AI-powered SLA breach predictions</p>
            </div>
            <button 
              onClick={fetchData}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="glass-panel rounded-xl p-5">
              <div className="text-sm text-[var(--text-muted)] uppercase mb-2">At Risk</div>
              <div className="text-3xl font-bold text-red-400">{riskTickets.length}</div>
              <div className="text-xs text-[var(--text-muted)]">Total predicted</div>
            </div>
            <div className="glass-panel rounded-xl p-5">
              <div className="text-sm text-[var(--text-muted)] uppercase mb-2">High Risk</div>
              <div className="text-3xl font-bold text-red-400">{highRisk.length}</div>
              <div className="text-xs text-[var(--text-muted)]">&gt;70% probability</div>
            </div>
            <div className="glass-panel rounded-xl p-5">
              <div className="text-sm text-[var(--text-muted)] uppercase mb-2">Medium Risk</div>
              <div className="text-3xl font-bold text-orange-400">{mediumRisk.length}</div>
              <div className="text-xs text-[var(--text-muted)]">40-70% probability</div>
            </div>
            <div className="glass-panel rounded-xl p-5">
              <div className="text-sm text-[var(--text-muted)] uppercase mb-2">Low Risk</div>
              <div className="text-3xl font-bold text-green-400">{lowRisk.length}</div>
              <div className="text-xs text-[var(--text-muted)]">&lt;40% probability</div>
            </div>
          </div>
          ...
          {riskTickets.length > 0 ? (
            ...
            <div className="glass-panel rounded-xl p-12 text-center">
              ...
              <p className="text-[var(--text-muted)]">No tickets at risk of SLA breach</p>
            </div>
          )}

          {/* Quick Links */}
          <div className="mt-6 grid grid-cols-3 gap-4">
            <Link to="/desk/ai/analytics" className="glass-panel rounded-xl p-4 hover:border-[var(--accent-gold)] transition text-center">
              <div className="text-2xl mb-2">📊</div>
              <div className="font-medium">AI Analytics</div>
            </Link>
            <Link to="/desk/ai/insights" className="glass-panel rounded-xl p-4 hover:border-[var(--accent-gold)] transition text-center">
              <div className="text-2xl mb-2">💡</div>
              <div className="font-medium">Smart Insights</div>
            </Link>
            <Link to="/desk/ai/agent-assist" className="glass-panel rounded-xl p-4 hover:border-[var(--accent-gold)] transition text-center">
              <div className="text-2xl mb-2">🎯</div>
