    ...
    <div className="min-h-screen bg-[var(--bg-0)]">
      ...
      <nav className="relative z-50 flex items-center justify-between px-6 py-4 border-b border-[var(--glass-border)] backdrop-blur-sm">
        <div className="flex items-center gap-4">
           ...
           <Link to="/portal/tickets" className="text-sm text-[var(--text-2)] hover:text-white ml-6">My Tickets</Link>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="max-w-xl mx-auto mb-12 relative">
          <input 
            type="text" 
            className="input-field w-full pl-10 py-3 rounded-full border-[var(--glass-border)] bg-black/20 focus:bg-black/50 transition-all font-medium"
            placeholder="Search for answers..."
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
          <span className="absolute left-4 top-3.5 text-[var(--text-2)]">🔍</span>
        </div>

        {selectedArticle ? (
          <div className="glass-panel p-8 rounded-xl animate-fade-in">
             <button onClick={() => setSelectedArticle(null)} className="text-sm text-[var(--text-2)] mb-4 hover:text-white">← Back to Search</button>
             <h2 className="text-2xl font-bold mb-4 text-[#60a5fa]">{selectedArticle.title}</h2>
             <div className="prose prose-invert max-w-none whitespace-pre-wrap font-light text-[var(--text-1)]">
               {selectedArticle.content}
             </div>
             <div className="mt-8 pt-6 border-t border-[var(--glass-border)] flex justify-between text-xs text-[var(--text-2)]">
             ...
             {loading ? <div className="spinner mx-auto"></div> : (
               <>
                 ...
                 {filteredArticles.length === 0 ? (
                    <div className="text-center text-[var(--text-2)]">No matches found.</div>
                 ) : (
                    <div className="grid gap-4">
                      {filteredArticles.map(article => (
                        ...
                        >
                          ...
                          <p className="text-sm text-[var(--text-2)] line-clamp-2">{article.content}</p>
                        </div>
                      ))}
                    </div>
                 )}
